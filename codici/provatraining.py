...
configs = [
    ...
    dict(pretrained = True, dropout=0.3, batch=32, freeze=0, weight_decay=0.01, hsv_h = 0.015, hsv_s = 0.7, hsv_v = 0.4, translate = 0.1, scale =0.5, fliplr =0.5),
...
for i, (yaml_path, cfg) in enumerate(zip(datasets, configs)):
    ...
    base_config = dict(
        ...
        cls=1.0,
        dfl=1.5,
        project='/gwpool/users/bscotti/tesi/dati/final_jet/curriculum_learning_jet_results_true',
        name=f'step_{i+1}'
    )
    ...
    model = YOLO(f'/gwpool/users/bscotti/tesi/dati/final_jet/curriculum_learning_jet_results_true/step_{i+1}/weights/best.pt').to(device)
